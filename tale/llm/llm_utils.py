...
class LlmUtil():
    ...
    def generate_start_location(self, location_desc: str, zone_info: dict, story_type: str, story_context: str, world_mood: str, world_info: str):
        """ Generate a location based on the current story context"""
        ...
        try:
            json_result = json.loads(parse_utils.sanitize_json(result))
            location = Location(name=json_result['name'], description=location_desc)
    ...
    def generate_start_zone(self, location_desc: str, story_type: str, story_context: str, world_mood: str, world_info: str) -> dict:
        """ Generate a zone based on the current story context"""
        prompt = self.pre_json_prompt
        prompt += self.zone_prompt.format(
            world_info=world_info,
